    ...
    relations: [
        {
            source: "M1",
            target: "M2",
            property: "P107i is current or former member of",
            label: "Works at",
            count: 1,
            fields: ["Works at"],
        },
...
describe("Structure view", () => {
    ...
    it("reveals its per-model sub-toolbar only on the Structure tab", () => {
        ...
        showStructure();
        expect(document.getElementById("ms-ge-struct-tools").hidden).toBe(
            false,
        );
        expect(document.getElementById("ms-ge-pane-structure").hidden).toBe(
            false,
        );
    ...
    it("departs a connector from the column gutter, never from the parent label", () => {
        ...
        flat.forEach(([x1, , bend, , x3]) => {
            ...
            const depth = [1, 2, 3, 4, 5, 6, 7].find(
                (d) => columnX(d) === childX,
            );
    ...
    it("sizes the frame so the rightmost target capsule is inside it", () => {
        ...
        const tx = (n) =>
            parseFloat(
                /translate\(([-\d.]+)/.exec(n.getAttribute("transform"))[1],
            );
        ...
        const rightmost = Math.max(
            ...caps.map(
                (c) =>
                    tx(c.parentNode) +
                    tx(c) +
                    +c.querySelector("rect").getAttribute("width"),
            ),
        );
    ...
    it("gives every disc a stroke so the fill is a redundant channel (WCAG 1.4.11)", () => {
        ...
        discs.forEach((d) =>
            expect(d.getAttribute("class")).toContain("ms-ge-st-disc"),
        );
    });
    ...
    it("survives its toolbar being used before a model is chosen", () => {
        expect(() => {
            ...
            document
                .getElementById("ms-ge-struct-semantic")
                .dispatchEvent(new Event("change"));
            document
                .getElementById("ms-ge-struct-cidoc")
                .dispatchEvent(new Event("change"));
    ...
    it("expands and collapses the whole tree", () => {
        ...
        document.getElementById("ms-ge-struct-collapse").click();
        expect(svg().querySelectorAll(".ms-ge-st-node").length).toBeLessThan(
            expanded,
        );
    });
    ...
    it("dims non-matching nodes on search rather than removing them", () => {
        ...
        expect(
            document.getElementById("ms-ge-struct-qcount").textContent,
        ).toBeTruthy();
    });
    ...
    it("refuses a javascript: scheme in a CIDOC URI from the payload", () => {
        ...
        expect(document.querySelector(".ms-ge-insp").textContent).toContain(
            "E67 Birth",
        );
    });
