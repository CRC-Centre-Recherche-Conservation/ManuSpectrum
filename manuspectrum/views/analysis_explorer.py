...
class AnalysisExplorerPageView(TemplateView):
    ...
    permission-checked explorer API. The page is never cached, because the
    header reads the user. A URL carrying a query is marked noindex; the
    canonical never carries one. ``explorer_connected`` hands the application
    the server's own ``is_connected`` answer for the reader.
    """
