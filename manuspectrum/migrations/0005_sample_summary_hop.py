...
Rewrites that single hop in the stored ``functions_x_graphs`` row of the Sample
model, and nothing else in it: the rest of the row is curated. Idempotent; a
...
def fix_sample_hop(apps, schema_editor):
    ...
    graph = GraphModel.objects.filter(slug="sample", isresource=True).first()
    if graph is None:
        return
...
class Migration(migrations.Migration):
    dependencies = [("manuspectrum", "0004_resource_summary")]
