...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def get_graph_name(graph_uuid):
            ...
            return GRAPH_ID_TO_NAME.get(str(graph_uuid), f"Unknown({str(graph_uuid)[:8]})")
        ...
        for gid in graph_filter:
            ...
            for idx, resource in enumerate(resources, 1):
                ...
                try:
                    source_name = resource.displayname() if callable(resource.displayname) else str(
                ...
                tiles = Tile.objects.filter(resourceinstance_id=resource.resourceinstanceid)
                    ...
                    for node_id, value in tile.data.items():
                        ...
                        for target_uuid in target_uuids:
                            try:
                                ...
                                try:
                                    target_name = target_resource.displayname() if callable(
                                        target_resource.displayname) else str(target_resource.displayname)
                                except:
                                    target_name = str(target_uuid)[:8]

                                target_graph_name = get_graph_name(target_resource.graph_id)
                                ...
                                # Type de relation
                                relation_type = f"{source_graph_name}→{target_graph_name}"
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                ...
                'source_id', 'source_name', 'source_graph',
                'target_id', 'target_name', 'target_graph',
                'tile_id', 'node_id', 'nodegroup_id',
                'rxr_exists', 'action', 'status'
            ]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            writer.writeheader()
            # Trier par relation_type puis par source_name
            sorted_relations = sorted(relations, key=lambda x: (x['relation_type'], x['source_name']))
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ Déjà dans ResourceXResource: {total_ok}"))
        ...
        for rel_type in sorted(stats_by_type.keys()):
            ...
            if stats['missing'] > 0:
                self.stdout.write(self.style.WARNING(f"  ⚠️  Manquantes: {stats['missing']}"))
            if stats['error'] > 0:
        ...
        self.stdout.write("2. Triez par 'relation_type' pour voir les types de relations")
        ...
        self.stdout.write("5. Testez: python manage.py import_relations_csv --input relations_export.csv --dry-run")
        self.stdout.write("6. Lancez: python manage.py import_relations_csv --input relations_export.csv")