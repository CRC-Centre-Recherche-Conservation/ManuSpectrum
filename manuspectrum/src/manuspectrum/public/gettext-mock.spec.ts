...
describe("vue3-gettext test double", () => {
    it("chooses the singular or the plural from the count", () => {
        ...
        expect($ngettext("%{n} result", "%{n} results", 0)).toBe(
            "%{n} results",
        );
        expect($ngettext("%{n} result", "%{n} results", 2)).toBe(
            "%{n} results",
        );
    });
    ...
    it("substitutes placeholders", () => {
        ...
        expect(interpolate("%{n} of %{ total }", { n: 3, total: 9 })).toBe(
            "3 of 9",
        );
    });

    it("escapes substituted values like the real plugin unless told not to", () => {
        const { interpolate } = useGettext();
        const values = { label: "d'heures & <or>" };
        expect(interpolate("%{label}", values)).toBe(
            "d&#039;heures &amp; &lt;or&gt;",
        );
        expect(interpolate("%{label}", values, true)).toBe("d'heures & <or>");
    });
