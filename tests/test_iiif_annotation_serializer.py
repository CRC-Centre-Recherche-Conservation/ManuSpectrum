...
class TestBuildMetadata(TestCase):
    ...
    def test_metadata_with_technique_concepts(self, mock_value):
        ...
        mock_value.objects.filter.return_value.select_related.return_value.values.return_value = [
            {"valueid": concept_id, "language_id": "en", "value": "XRF", "concept__conceptid": uuid.uuid4()}
        ]
        ...
        technique_entry = next((m for m in result if "Technique" in str(m["label"])), None)
    ...
    def test_metadata_with_instrumental_metadata(self):
        ...
        tiles_data = {
            meta_node: {"en": {"value": "Voltage: 40kV, Current: 30mA"}}
        }

        result = self.serializer._build_metadata(tiles_data)

        instr_entry = next((m for m in result if "Instrumental" in str(m["label"])), None)
...
class TestBatchProcessing(TestCase):
    ...
    def test_batch_to_representation_processes_multiple(
        ...
        annotations_data = [
            ...
            {"target": "https://example.com/canvas/2#xywh=0,0,50,50", "resource_id": str(uuid.uuid4())},
        ]
    ...
    def test_prefetch_all_data_called_in_batch(self, mock_tile):
        ...
        with patch.object(self.serializer, '_prefetch_all_data') as mock_prefetch:
            self.serializer.batch_to_representation([
                {"target": "https://example.com/canvas/1", "resource_id": str(uuid.uuid4())}
            ])
...
class TestPrefetchMethods(TestCase):
    ...
    def test_prefetch_tiles(self, mock_tile):
        ...
        self.assertEqual(self.serializer._tiles_cache[resource_id][node_id], "test_value")
    ...
    def test_batch_load_concepts(self, mock_value):
        ...
        mock_value.objects.filter.return_value.select_related.return_value.values.return_value = [
            {"valueid": concept_id, "language_id": "en", "value": "English Label", "concept__conceptid": concept_uuid},
            {"valueid": concept_id, "language_id": "fr", "value": "French Label", "concept__conceptid": concept_uuid},
        ]

        self.serializer._batch_load_concepts([concept_id])

        self.assertIn(concept_id, self.serializer._concept_cache)
        self.assertEqual(self.serializer._concept_cache[concept_id]["labels"]["en"], "English Label")
        self.assertEqual(self.serializer._concept_cache[concept_id]["labels"]["fr"], "French Label")
    ...
    def test_batch_load_resources(self, mock_resource):
        ...
        self.assertEqual(self.serializer._resource_cache[resource_id]["labels"]["en"], "Test Resource")
    ...
    def test_batch_load_manifests(self, mock_manifest):
        ...
        self.assertEqual(self.serializer._manifest_cache[manifest_url]["label"], "Test Manifest")
...
class TestFormatMetadataValue(TestCase):
    ...
    def test_format_acquisition_date(self):
        ...
        result = self.serializer._format_metadata_value("2024-01-15", "acquisition_date")
